...
C.bar_candidate=c(6,3,1.5,2.6,2,1.8,1.2,1,0.9,1.3)

simulations = 200
end = matrix(0, simulations, 4)
names(end) = c("fps", "fn", "hamming", "est.time")
...
#  end[trial,3] = jw.elaps
  ...
  beta.thresholding=beta_threshold(result[-1],factor=3,C.bar,lambda.hat)
  ...
  fp.counts_thr=fp.test(beta.star,beta.thresholding)
  fn.counts_thr=fn.test(beta.star,beta.thresholding)
  
#   fp.counts = fp.test(beta.star, result[-1])
#   fn.counts = fn.test(beta.star, result[-1])
  end[trial,1] = fp.counts_thr
  end[trial,2] = fn.counts_thr
  end[trial,3] = fp.counts_thr+fn.counts_thr
  end[trial,4] = as.numeric(jw.elaps)
#   
#   which(result[-1]!=0)
#   which(beta.star!=0)
   print(trial)
  cat("fp.counts_thr=",fp.counts_thr,"\n")
  cat("fn.counts_thr=",fn.counts_thr,"\n") 
  print(jw.elaps)
#   t1=Sys.time()
