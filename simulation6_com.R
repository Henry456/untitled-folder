...
beta_star = function(p, sparsity, mu.beta, sigma = 0.02^2){
  beta = rep(0,p)
  index.zero = sample(1:p, p-sparsity, replace=FALSE)
  betas = log(rnorm(sparsity, mu.beta, 0.02^2))
...
Ys = function(beta.star, X.support, support){
  dim(X.support)
  ones = rep(1, nrow(X.support))
  u =ones %*% (X.support %*% beta.star[support]) / nrow
  dim(X.support); length(beta.star[support]); length(ones)
...
design_matrix = function(nrow, ncol, beta.star){
...
design_matrix1 = function(nrow, ncol, beta.star){
  support = (beta.star != 0)
  X = matrix(0, nrow, ncol) 
  Sigma=matrix(0,ncol,ncol)
...
design_matrix2 = function(nrow, ncol, beta.star){
  support = (beta.star != 0)
  X = matrix(0, nrow, ncol) 
  Sigma=matrix(0,ncol,ncol)
  for(i in 1:ncol){
...
design_matrix3 = function(nrow, ncol, beta.star){
  support = (beta.star != 0)
  X = matrix(0, nrow, ncol) 
  Sigma=matrix(0,ncol,ncol)
  for(i in 1:ncol){
...
AIC_fun = function(data, grid){
  ...
  index.lambda_hat = which(AIC.values == min(AIC.values))
...
BIC_fun = function(data, grid){
  ...
  index.lambda_hat = which(BIC.values == min(BIC.values))
  lambda.hat = grid[index.lambda_hat]
  return(lambda.hat)
}
...
nrow = 1000 # n = 200 is high-dimension, n = 1000 is low-dimension
ncol = 500;

sparsity_vec = c(1, 5, 10, 20)
sparsity = 5
...
for(mu.beta in mu.beta.lows){
    for(sparsity in sparsity_vec){  
        ...
        set.seed(1:(length(mu.beta.lows)*length(sparsity_vec)))
        ...
        for(trial in 1:simulations){ 
              try( 
                  { 
                    ...
                    set.seed(trial + 100)
                    designMs = list(m1 = design_matrix(nrow, ncol, beta.star), m2 = design_matrix1(nrow, ncol, beta.star), 
                                    m3 = design_matrix2(nrow, ncol, beta.star), m4 = design_matrix3(nrow, ncol, beta.star))
                    for(indOfList in seq_along(designMs)){  
                        cat("n= ", nrow, "p= ", ncol, "mu.beta= ", mu.beta, "s= ", sparsity, "design.matrix= desM ",  indOfList, "\n")
                        desM = designMs[[indOfList]]
                        data = desM$data
                        ...
                        ends.bic[[indOfList]] = end.bic
                        
                        cat("designM ", indOfList, ") trial ", trial, " ends. ",  "(av)h1 and h2 are ", end.av[trial, 3]," and ", end.av[trial, 4], "\n")
                        cat("designM ", indOfList, ") trial ", trial, " ends. ",  "(tr)h1 and h2 are ", end.tr[trial, 3], " and ", end.tr[trial, 4], "\n")
                        cat("designM ", indOfList, ") trial ", trial, " ends. ",  "(cv)h1 and h2 are = ", end.cv[trial, 3], " and ", end.cv[trial, 4], "\n")
                        cat("designM ", indOfList, ") trial ", trial, " ends. ",  "(aic)h1 and h2 are = ", end.aic[trial, 3], " and ", end.aic[trial, 4], "\n")
                        cat("designM ", indOfList, ") trial ", trial, " ends. ",  "(bic)h1 and h2 are = ", end.bic[trial, 3], " and ", end.bic[trial, 4], "\n")
                    }
                  }
              )
            }
        ...
        for(i in 1:4){
          end.matrix = data.frame(ends.av[[i]], ends.tr[[i]], ends.cv[[i]], ends.aic[[i]], ends.bic[[i]])
          myfilename = paste0("dm", i, "lr", nrow, "c", ncol,  "lu", mu.beta, "s", sparsity, ".csv")
...
###########################################################Same method, being bored to make a function
#The only difference is the mu.beta and nrow which are generated for high dimension scales.

mu.beta.highs = c(log(2.5), log(3)) #high dimension corresponding to n = 200
nrow = 200 # n = 200 is high-dimension, n = 1000 is low-dimension

for(mu.beta in mu.beta.highs){
  for(sparsity in sparsity_vec){  
