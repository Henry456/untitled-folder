...
nrow = 100; ncol = c(100, 200, 500); 
sparsity=c(6, 5, 10, 15)
rho = c(0, 0.25, 0.5)
possibilities = length(nrow)*length(ncol)*length(sparsity)*length(rho)
index.matrix = expand.grid(nrow, ncol, rho, sparsity)
colnames(index.matrix) = c("nrow", "ncol", "rho", "sparsity")



for(i in 1:possibilities){
  
    input = index.matrix[i, ] 
    input.nrow = input$nrow 
    input.ncol = input$ncol
    input.rho = input$rho
    input.sparsity = input$sparsity
    
    beta.star = beta_star(input.ncol, input.sparsity)
    
    lambda.max=10*sqrt(log(input.ncol)/input.nrow)
    grid = seq(0.0001*lambda.max,lambda.max,length.out=500)
    
    C.bar=c(2.5,2,1.8,1.5, 1.4, 1.3, 1.2, 1, 0.9)
    ...
    for(cbar in C.bar){
        ...
        for(trial in 1:simulations){
          try(
            {
            cat("nrow, ncol, rho, sparsity are ", input.nrow, input.ncol, input.rho, input.sparsity, "\n")
            cat("trial ", trial, " starts and Cbar = ", cbar, "\n")
            #define random design matrix
            set.seed(trial)
            
            desM = design_matrix(input.nrow, input.ncol, input.rho, beta.star)
            data = desM$data
        ...
        myfilename = paste0("Cbar" , cbar,
                            "r", input.nrow,
                            "c", input.ncol, 
                            "o", input.rho,
                            "s", input.sparsity,".csv")
